...
export default class UserService implements IUserService {
    ...
    createUser = async (userBodyReq: IUser) => {
        try {
            ...
            let message = 'Successfully Registered the account! Please Verify your userName.';
            ...
            userBody.userName_verified = userConstant.USERNAME_VERIFIED_FALSE;
    ...
    isUserNameExists = async (userName: string) => {
        const message = 'UserName found!';
        if (!(await this.userDao.isUserNameExists(userName))) {
            return responseHandler.returnError(httpStatus.BAD_REQUEST, 'UserName not Found!!');
        }
